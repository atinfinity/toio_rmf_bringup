# 章8: フリートアクション(perform_action)

← [前章: 搬送とワークセル](07_delivery.md) | [目次](README.md) | 次章: [可視化とダッシュボード →](09_visualization.md)
...
delivery(章7)がワークセル側で完結してキューブが無反応なのに対し、
これは**キューブ側の演出**にあたる。
...
- (実機なら)LEDが色づき(pickup=緑 / dropoff=青)、効果音が鳴る。
  ...
  アクション実行**は同じように起きる。この「見た目」の差こそが章10で実機に
  移る動機になる。
...
**フリートの振る舞いは設定ファイルで決まっている**という感覚 ── 章6で
...
- **delivery(章7)と perform_action(本章)は「荷役の見せ方」が逆**。
...
- 章6・本章で**設定ファイルを2回いじった**。フリートの人格(いつ充電するか、
...
3. delivery(章7)と perform_action(本章)で、「荷役をするのは誰か」の
   違いを一文で説明する。
