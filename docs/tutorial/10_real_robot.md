...
← [前章: 可視化とダッシュボード](09_visualization.md) | [目次](README.md)
...
ここまでの章はすべてこの章の準備だった。**タスク操作(章2〜8)は
`--use_sim_time` の有無以外そのまま通る** ── それを実機で確かめる。

## 変わらないこと / 変わること

[章1](01_architecture.md)の三層モデルを思い出す。**差し替わるのは一番下の
ロボット層(③)だけ**。RMFコア(①)とフリートアダプタ(②)、つまり入札・
交通調停・充電・タスクの仕組みは**まったく同じ**。

| | シミュレーション(章0〜9) | 実機(この章) |
|---|---|---|
...
「位置とバッテリは③→②へ別経路で上がる」という[章1](01_architecture.md)の話が、
...
[章5](05_traffic.md)と[docs/TASKS.md](../TASKS.md)で見たとおり。チャージャーを
ループ上でなくスパーの先に置いているのは、通過するだけのロボットが駐機中の
相手に突っ込まないため。

## 実機で試す(sim編のコマンドがそのまま通る)

`--use_sim_time` を**外す**だけで、章2〜8のコマンドがそのまま使える。A4の
頂点名(`patrol_A` / `patrol_B` / `charger_1` / `charger_2`)を使う:

```bash
# patrol(章3 と同じ。--use_sim_time が無いだけ)
...
> 角接触)。確実な非接触が要る検証はA3、という判断は[章5](05_traffic.md)で
> 見たとおり。2台同時は接触リスクを認識のうえで。

実機ではここで**LEDと効果音**([章8](08_fleet_action.md))が実際に見える。
バッテリも実測値([章6](06_battery_charge.md)、10%刻み)で動く。シミュレー
...
- [ ] patrolタスク完走(1台・3周)← [章3](03_patrol.md)
- [ ] 2台のフリート登録と位置報告(RVizが実位置と一致)← [章1](01_architecture.md)
- [ ] 1台ずつの `go_to_place`(順次)← [章2](02_go_to_place.md)
- [ ] バッテリ離散値(10%刻み)の実測確認 ← [章6](06_battery_charge.md)
- [ ] 低バッテリ時のChargeBattery発行・チャージャー帰還 ← [章6](06_battery_charge.md)
- [ ] タスクキャンセル → 再投入 ← [章6](06_battery_charge.md)
...
```
章2  1台を1回動かす         ── タスク→Nav2委譲の縦串
章3  巡回とnavグラフ         ── フリートの地図
章4  入札                   ── 誰がやるか(タスク割当)      ┐
章5  交通調停               ── 道をどう分けるか              ├ フリート処理の核心
章6  バッテリ・充電          ── いつ休むか(自己管理)        ┘
章7-8 荷役・演出            ── 移動以外のタスク
章9  可視化                 ── 内部状態を見る
章10 実機                   ── ③だけ差し替え、①②はそのまま
```
