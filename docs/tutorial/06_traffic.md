...
```
大域(RMFコア)   rmf_traffic_schedule / rmf_traffic_blockade
                  … 経路を時空間で予約。「このレーンをこの時刻に使う」を
                    フリート全体で調整し、譲り合いや順番待ちを決める
─────────────────────────────────────────────────
局所(Nav2)      peer_robot_costmap_publisher
                  … 相手ロボットの現在位置をコストマップに障害物として
                    描き込む。目の前の相手をその場で避ける
```
